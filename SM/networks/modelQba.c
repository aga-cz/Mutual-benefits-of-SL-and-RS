...
int N, X, L, Cs, Ci, Cr, Q, m;
double r;

N = atoi(argv[1]); // number of agents
int T; // number of time steps
T=20*N;
m = atoi(argv[2]); // m in BA net, k=2m
...
int total_edges = 0;          // Total number of edges (used for preferential attachment)
for(i=0;i<N;i++){degree[i]=0;}

//// build BA network
 if (m < 1 || m >= N) {
        printf("Error: Barabasi-Albert requires 1 <= m < n.\n");
        return -1;
    }


    // Step 1: Start with a complete graph of (m + 1) nodes
    for(i = 0; i <= m; i++){
        for(j = i + 1; j <= m; j++){
            AA[i][j] = 1;
            AA[j][i] = 1;
    ...
    for (int new_node = m + 1; new_node < N; new_node++) {
        ...
        // Attach m edges using preferential attachment
        while (edges_added < m) {
            int existing_node = rand() % new_node;

            // Preferential attachment: connect based on degree
            double attachment_prob = (double)degree[existing_node] / (2.0 * total_edges);
            los=frand();
            if (los < attachment_prob && AA[new_node][existing_node] == 0) {
                AA[new_node][existing_node] = 1;
                AA[existing_node][new_node] = 1;
                degree[new_node]++;
                degree[existing_node]++;
                total_edges++;
                edges_added++;
            }
        }
...
sprintf(rs, "%savg_N_%d_m_%d_X_%d_L_%d_Cs_%d_Ci_%d_Cr_%d_r_%.1lf__%d.csv",out_dir_t,N,m,X,L,Cs,Ci,Cr,r,Q);
