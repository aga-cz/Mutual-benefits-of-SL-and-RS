...
int N, X, L, Cs, Ci, Cr, Q;
double pER, r;

N = atoi(argv[1]); // number of agents
int T; // number of time steps
T=20*N;
pER = atof(argv[2]); // p in ER net
...
int lvl, item, user, i, j, step, maxP;
		...
		if(los<r || r==1.0){//RS
			...
			if(UB[user]>=Cs){
				...
				if(num_u>0){
					...
					if(UL[choices[i]]>=lvl){
						UB[user]-=Cs;
						item = UI[choices[i]][lvl]; //note that item here actually means payoff
						UI[user][lvl] = item;
						UL[user]+=1;
...
sprintf(rs, "%savg_N_%d_ER_%.3lf_X_%d_L_%d_Cs_%d_Ci_%d_Cr_%d_r_%.1lf__%d.csv",out_dir_t,N,pER,X,L,Cs,Ci,Cr,r,Q);
